...
RDF_ACTION_POS_GAIN="${RDF_ACTION_POS_GAIN:-0.40}"
...
RDF_ACTION_POS_YAW_OFFSET_DEG="${RDF_ACTION_POS_YAW_OFFSET_DEG:-0}"
...
RDF_DIRECT_EE_MAX_STEP_M="${RDF_DIRECT_EE_MAX_STEP_M:-0.04}"
RDF_DIRECT_EE_MAX_ROT_STEP_RAD="${RDF_DIRECT_EE_MAX_ROT_STEP_RAD:-0.12}"
RDF_DIRECT_EE_SMOOTHING_ALPHA="${RDF_DIRECT_EE_SMOOTHING_ALPHA:-0.50}"
RDF_DIRECT_EE_DEADZONE_M="${RDF_DIRECT_EE_DEADZONE_M:-0.003}"
RDF_DIRECT_EE_WORKSPACE_RADIUS_M="${RDF_DIRECT_EE_WORKSPACE_RADIUS_M:-0.35}"
RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES="${RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES:-3}"
RDF_RAW_WRIST_REACQUIRE_STABLE_M="${RDF_RAW_WRIST_REACQUIRE_STABLE_M:-0.03}"
...
RDF_TASK_GUIDANCE_PANEL_SIZE="${RDF_TASK_GUIDANCE_PANEL_SIZE:-1.25}"
...
RDF_GUIDANCE_PEG_TIP_DISTANCE_MAX="${RDF_GUIDANCE_PEG_TIP_DISTANCE_MAX:-0.030}"
RDF_GUIDANCE_PEG_AXIS_ALIGNMENT_MAX_RAD="${RDF_GUIDANCE_PEG_AXIS_ALIGNMENT_MAX_RAD:-0.40}"
RDF_GUIDANCE_INSERTION_DEPTH_MIN="${RDF_GUIDANCE_INSERTION_DEPTH_MIN:-0.010}"
RDF_INSERTION_AXIS_WORLD="${RDF_INSERTION_AXIS_WORLD:-}"
RDF_HOLE_TARGET_LOCAL_OFFSET="${RDF_HOLE_TARGET_LOCAL_OFFSET:-0,0,0.025}"
RDF_SUCCESS_READY_HOLD_SEC="${RDF_SUCCESS_READY_HOLD_SEC:-0.5}"
...
RDF_LIVE_CURATION_MAX_RETARGETING_JUMP="${RDF_LIVE_CURATION_MAX_RETARGETING_JUMP:-3.50}"
...
RDF_AUTO_RECENTER_VALID_FRAMES="${RDF_AUTO_RECENTER_VALID_FRAMES:-5}"
RDF_RECENTER_MODE="${RDF_RECENTER_MODE:-robot_start_box}"
RDF_RECENTER_BOX_CENTER="${RDF_RECENTER_BOX_CENTER:-}"
RDF_RECENTER_BOX_CENTER_SOURCE="${RDF_RECENTER_BOX_CENTER_SOURCE:-hole_target_approach}"
RDF_RECENTER_BOX_APPROACH_OFFSET="${RDF_RECENTER_BOX_APPROACH_OFFSET:-0,0,0.08}"
RDF_RECENTER_BOX_RANDOM_OFFSET="${RDF_RECENTER_BOX_RANDOM_OFFSET:-0.02,0.02,0.01}"
RDF_RECENTER_BOX_HALF_EXTENTS="${RDF_RECENTER_BOX_HALF_EXTENTS:-0.07,0.07,0.07}"
RDF_RECENTER_BOX_VISUAL="${RDF_RECENTER_BOX_VISUAL:-0}"
RDF_BLOCK_TELEOP_UNTIL_RECENTER="${RDF_BLOCK_TELEOP_UNTIL_RECENTER:-1}"
RDF_RECENTER_SETUP_CONTROL="${RDF_RECENTER_SETUP_CONTROL:-1}"
...
Environment:
  ...
  RDF_ACTION_POS_GAIN  Default: 0.40
  ...
  RDF_ACTION_POS_YAW_OFFSET_DEG Default: 0 (OpenXR Y-up horizontal X/Z plane rotation before axis map)
  RDF_ACTION_POS_AXIS_MAP Default: x,z,y (OpenXR Y-up -> Isaac Z-up)
  ...
  RDF_DIRECT_EE_MAX_STEP_M Default: 0.04
  RDF_DIRECT_EE_MAX_ROT_STEP_RAD Default: 0.12
  RDF_DIRECT_EE_SMOOTHING_ALPHA Default: 0.50
  RDF_DIRECT_EE_DEADZONE_M Default: 0.003
  RDF_DIRECT_EE_WORKSPACE_RADIUS_M Default: 0.35
  RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES Default: 3 (raw-wrist valid-to-valid spike 후 stable frame 확인 수)
  RDF_RAW_WRIST_REACQUIRE_STABLE_M Default: 0.03 (reacquire 후보 frame 간 허용 raw wrist 이동)
  ...
  RDF_TASK_GUIDANCE_PANEL_SIZE Default: 1.25
  RDF_TASK_GUIDANCE_PANEL_SOURCE Default: /_xr/stage/xrCamera
  RDF_TASK_GUIDANCE_PANEL_TRANSLATION Default: 0.00,0.10,-1.60
  RDF_TASK_GUIDANCE_PANEL_LOOK_AT_CAMERA Default: 1
  RDF_TASK_GUIDANCE_PANEL_BACKGROUND_ALPHA Default: 0.90
  RDF_TASK_GUIDANCE_PANEL_ANCHOR_MODE Default: upstream_instruction
  RDF_TASK_GUIDANCE_PRIMITIVE_FALLBACK Default: 0 (XR text widget 실패 시 crude primitive fallback)
  RDF_GUIDANCE_PEG_TIP_DISTANCE_MAX Default: 0.030 (peg tip to hole distance gate)
  RDF_GUIDANCE_PEG_AXIS_ALIGNMENT_MAX_RAD Default: 0.40 (peg axis misalignment gate)
  RDF_GUIDANCE_INSERTION_DEPTH_MIN Default: 0.010 (minimum insertion depth for SUCCESS_READY)
  RDF_INSERTION_AXIS_WORLD Optional insertion depth projection axis, e.g. 0,0,1 for visual practice
  RDF_SUCCESS_READY_HOLD_SEC Default: 0.5
  ...
  RDF_LIVE_CURATION_MAX_RETARGETING_JUMP Default: 3.50
  ...
  RDF_AUTO_RECENTER_VALID_FRAMES Default: 5
  RDF_RECENTER_MODE Default: robot_start_box (first_valid_hand|robot_start_box)
  RDF_RECENTER_BOX_CENTER Optional x,y,z. Overrides center source when set.
  RDF_RECENTER_BOX_CENTER_SOURCE Default: hole_target_approach (hole_target_approach|initial_robot_pose)
  RDF_RECENTER_BOX_APPROACH_OFFSET Default: 0,0,0.08
  RDF_RECENTER_BOX_RANDOM_OFFSET Default: 0.02,0.02,0.01 (episode/reset마다 1회 uniform sample)
  RDF_RECENTER_BOX_HALF_EXTENTS Default: 0.07,0.07,0.07
  RDF_RECENTER_BOX_VISUAL Default: 0 (1이면 HMD/AR scene에 recenter wire box 표시)
  RDF_BLOCK_TELEOP_UNTIL_RECENTER Default: 1
  RDF_RECENTER_SETUP_CONTROL Default: 1 (recenter 전 박스 진입용 setup 이동만 허용)
...
if [[ "$SKIP_ISAAC" == "1" ]]; then
  ...
  log "RDF_ACTION_POS_YAW_OFFSET_DEG=$RDF_ACTION_POS_YAW_OFFSET_DEG"
  ...
  log "RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES=$RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES"
  log "RDF_RAW_WRIST_REACQUIRE_STABLE_M=$RDF_RAW_WRIST_REACQUIRE_STABLE_M"
  ...
  log "RDF_TASK_GUIDANCE_PANEL_LOOK_AT_CAMERA=$RDF_TASK_GUIDANCE_PANEL_LOOK_AT_CAMERA"
  log "RDF_TASK_GUIDANCE_PANEL_BACKGROUND_ALPHA=$RDF_TASK_GUIDANCE_PANEL_BACKGROUND_ALPHA"
  log "RDF_TASK_GUIDANCE_PANEL_ANCHOR_MODE=$RDF_TASK_GUIDANCE_PANEL_ANCHOR_MODE"
  ...
  log "RDF_HOLE_TARGET_LOCAL_OFFSET=$RDF_HOLE_TARGET_LOCAL_OFFSET"
  ...
  log "RDF_RECENTER_MODE=$RDF_RECENTER_MODE"
  log "RDF_RECENTER_BOX_CENTER=${RDF_RECENTER_BOX_CENTER:-<from_center_source>}"
  log "RDF_RECENTER_BOX_CENTER_SOURCE=$RDF_RECENTER_BOX_CENTER_SOURCE"
  log "RDF_RECENTER_BOX_APPROACH_OFFSET=$RDF_RECENTER_BOX_APPROACH_OFFSET"
  log "RDF_RECENTER_BOX_RANDOM_OFFSET=$RDF_RECENTER_BOX_RANDOM_OFFSET"
  log "RDF_RECENTER_BOX_HALF_EXTENTS=$RDF_RECENTER_BOX_HALF_EXTENTS"
  log "RDF_RECENTER_BOX_VISUAL=$RDF_RECENTER_BOX_VISUAL"
  log "RDF_BLOCK_TELEOP_UNTIL_RECENTER=$RDF_BLOCK_TELEOP_UNTIL_RECENTER"
  log "RDF_RECENTER_SETUP_CONTROL=$RDF_RECENTER_SETUP_CONTROL"
  ...
  RDF_RECORD=1 \
    ...
    RDF_ACTION_POS_YAW_OFFSET_DEG="$RDF_ACTION_POS_YAW_OFFSET_DEG" \
    ...
    RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES="$RDF_RAW_WRIST_REACQUIRE_VALID_FRAMES" \
    RDF_RAW_WRIST_REACQUIRE_STABLE_M="$RDF_RAW_WRIST_REACQUIRE_STABLE_M" \
    ...
    RDF_TASK_GUIDANCE_PANEL_LOOK_AT_CAMERA="$RDF_TASK_GUIDANCE_PANEL_LOOK_AT_CAMERA" \
    RDF_TASK_GUIDANCE_PANEL_BACKGROUND_ALPHA="$RDF_TASK_GUIDANCE_PANEL_BACKGROUND_ALPHA" \
    RDF_TASK_GUIDANCE_PANEL_ANCHOR_MODE="$RDF_TASK_GUIDANCE_PANEL_ANCHOR_MODE" \
    ...
    RDF_INSERTION_AXIS_WORLD="$RDF_INSERTION_AXIS_WORLD" \
    RDF_HOLE_TARGET_LOCAL_OFFSET="$RDF_HOLE_TARGET_LOCAL_OFFSET" \
    ...
    RDF_RECENTER_MODE="$RDF_RECENTER_MODE" \
    RDF_RECENTER_BOX_CENTER="$RDF_RECENTER_BOX_CENTER" \
    RDF_RECENTER_BOX_CENTER_SOURCE="$RDF_RECENTER_BOX_CENTER_SOURCE" \
    RDF_RECENTER_BOX_APPROACH_OFFSET="$RDF_RECENTER_BOX_APPROACH_OFFSET" \
    RDF_RECENTER_BOX_RANDOM_OFFSET="$RDF_RECENTER_BOX_RANDOM_OFFSET" \
    RDF_RECENTER_BOX_HALF_EXTENTS="$RDF_RECENTER_BOX_HALF_EXTENTS" \
    RDF_RECENTER_BOX_VISUAL="$RDF_RECENTER_BOX_VISUAL" \
    RDF_BLOCK_TELEOP_UNTIL_RECENTER="$RDF_BLOCK_TELEOP_UNTIL_RECENTER" \
    RDF_RECENTER_SETUP_CONTROL="$RDF_RECENTER_SETUP_CONTROL" \
