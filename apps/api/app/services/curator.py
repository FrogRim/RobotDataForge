...
def trajectory_signature(
    trajectory: dict[str, Any], points: int = 16
) -> list[list[float]]:
...
def is_duplicate(
    signature: list[list[float]], seen: list[list[list[float]]], threshold: float = 0.03
) -> bool:
    if not signature:
        return False
    for other in seen:
        if len(other) != len(signature):
            continue
        avg = sum(euclidean(a, b) for a, b in zip(signature, other, strict=True)) / len(
            signature
        )
        if avg <= threshold:
            return True
...
def _append_unique_reason(rejection_reasons: list[str], reason: Any) -> None:
...
def _evaluation_gate_reasons(evaluation: dict[str, Any]) -> list[str]:
    """Preserve evaluator data-quality reasons without changing legacy replay policy."""
    ...
    if isinstance(data_quality, dict):
        for reason in data_quality.get("quality_failure_reasons") or []:
...
def curate_episodes_with_reasons(
    ...
    for episode in episodes:
        ...
        usability = episode.get("data_usability") or summary.get("data_usability") or {}
        ...
        for reason in _evaluation_gate_reasons(evaluation):
            _append_unique_reason(rejection_reasons, reason)
        data_usability_score = usability.get(
            "score", evaluation.get("data_usability_score")
        )
        if (
            data_usability_score is not None
            and float(data_usability_score) < min_data_usability_score
        ):
            rejection_reasons.append("LOW_DATA_USABILITY_SCORE")
        if usability.get("usable") is False:
            rejection_reasons.extend(
                str(reason) for reason in usability.get("rejection_reasons", [])
            )
